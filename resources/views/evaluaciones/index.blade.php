...
<div class="flex justify-between items-center mb-6">
    <h2 class="text-2xl font-bold text-gray-800">Mis Evaluaciones</h2>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3">
                    @php $nivel = $evaluacion->nivel_riesgo; @endphp

                    <span class="px-2 py-1 rounded text-white text-xs
                        {{ $nivel === 'Riesgo inapreciable' ? 'bg-gray-500' :
                           ($nivel === 'Riesgo bajo' ? 'bg-green-500' :
                           ($nivel === 'Riesgo medio' ? 'bg-yellow-500' :
                           ($nivel === 'Riesgo alto' ? 'bg-orange-500' : 'bg-red-600'))) }}">
                        {{ $nivel ?? '-' }}
                    </span>
                </td>

                <td class="px-4 py-3">{{ optional($evaluacion->fecha)->format('d/m/Y') ?? $evaluacion->created_at->format('d/m/Y') }}</td>
